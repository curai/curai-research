...
def generate_violations(
    ...
    # run.log({
    #     'scenarios': wandb.Table(dataframe=scenarios),
    # })

    output = []
    # violations
    scenario_cycle = {rule_num:cycle(scenario_list) for rule_num, scenario_list in scenario_dict.items()}
    rule_cycle = cycle(range(len(rule_dict) - 1))
    # access first scenario in each rule
    # scenario_cycle = {rule_num:cycle([scenario_list[0]]) for rule_num, scenario_list in scenario_dict.items()}
    for i in range(0, num_samples): 
        ...
        output.append({
            ...
            'scenario': scenario,
            # 'rule_scenario': f'r{str(rule_num)}s{str(scenario_dict[rule_num].index(scenario))}',
...
#     violation_path = "multi-rule-guardrail/sgd/prompts/violation.jinja",
#     rules_path="multi-rule-guardrail/sgd/prompts/rules.csv",
#     scenario_prompt_path="multi-rule-guardrail/sgd/prompts/scenario_generator.jinja",
#     # scenario_csv_path="multi-rule-guardrail/sgd/output/restaurant_scenarios.csv",
...
generate_AME_nonviolations("multi-rule-guardrail/sgd/output/convos_violations_restaurants_3-22.csv",
    "multi-rule-guardrail/sgd/prompts/ame_violation_restaurants.jinja",
    "restaurants",
...
# generate_nonviolations(55, "multi-rule-guardrail/sgd/prompts/nonviolation_restaurants.jinja", "multi-rule-guardrail/sgd/output")