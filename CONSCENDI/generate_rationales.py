...
parser.add_argument('--domain', type=str, default='r', help='domain to generate rationales for')
...
for i, row in df.iterrows():
    ...
    if row['generation'] == 'v':
        ...
        prompt = load_template("multi-rule-guardrail/sgd/prompts/rationale_generator_violation.jinja").render(
        ...
        prompt = load_template("multi-rule-guardrail/sgd/prompts/rationale_generator_nonviolation.jinja").render(
            role = rule_map[domain]['role'],
            rules_list = rule_map[domain]['rules_list'],
            last_two_turns = last_two_turns,
        )
    ...
    if row['completion'] == ' n':
        completion += ' Thus, there is no violation, so n.'
